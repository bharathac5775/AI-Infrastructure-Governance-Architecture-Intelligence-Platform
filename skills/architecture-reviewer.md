...
NO BUNDLE ECHOES: Do NOT create a cross-cutting gap by bundling multiple agent findings into one item. For example, if Security flagged "S3 no encryption" and Reliability flagged "no PITR", do NOT create a gap called "Data Protection" that lists both. Each agent already reported and scored those individually. A valid cross-cutting gap identifies a NEW emergent risk from the INTERACTION between findings — not a summary of existing findings. Ask yourself: "Does this gap describe something no individual agent could have caught alone?" If not, do not include it.
