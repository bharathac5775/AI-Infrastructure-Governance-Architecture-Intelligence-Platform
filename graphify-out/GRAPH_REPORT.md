# Graph Report - AI-Infrastructure-Governance-Architecture-Intelligence-Platform  (2026-07-15)

## Corpus Check
- 84 files · ~87,828 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1366 nodes · 3451 edges · 84 communities (82 shown, 2 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 445 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b8e036f0`
...
1. `_f()` - 122 edges
2. `Severity` - 114 edges
3. `remediate_sync()` - 112 edges
4. `Finding` - 108 edges
5. `AgentReport` - 79 edges
...
Cohesion: 0.05
Nodes (44): AnalysisReport, Finding, _agent_findings_by_prefix(), _agent_has_data(), _all_deterministic_findings(), compute_drift(), _delta(), _finding_signature() (+36 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (54): _companion_template(), _detect_json_indent(), _dump_docs_for_kind(), _ensure_pod_spec(), _filename_kind(), _find_workload_doc(), _fix_k8s(), _fix_tf() (+46 more)

### Community 2 - "Community 2"
Cohesion: 0.06
Nodes (45): compare_reports_endpoint(), delete_report_endpoint(), drift_endpoint(), export_report_pdf(), _flat_findings(), get_report_endpoint(), health_check(), list_reports_endpoint() (+37 more)

### Community 3 - "Community 3"
Cohesion: 0.06
Nodes (23): Any, extract_k8s_resources(), get_pod_spec(), get_resource_name(), Group parsed K8s documents by resource kind., Get pod spec from various resource types., Get the name of a K8s resource., extract_tf_resources() (+15 more)

### Community 4 - "Community 4"
Cohesion: 0.11
Nodes (37): analyze_cost(), Run cost analysis using rules + LLM reasoning., analyze_reliability(), Run reliability analysis using rules + LLM reasoning., analyze_security(), _detect_infra_type(), Detect whether files are kubernetes, terraform, mixed, or none (non-infra)., Run security analysis using rules + LLM reasoning. (+29 more)

### Community 5 - "Community 5"
Cohesion: 0.09
Nodes (34): Synchronous wrapper around :func:`remediate` for tests / scripts.      Spawns a, remediate_sync(), The Reliability/Cost agents emit findings with category="ai-analysis"     that f, Real failure mode from samples/vulnerable-infra.tf: Reliability         Agent em, The inference helper must be a no-op for findings with a real         rule-engin, When the inference table doesn't match (novel title, unknown         resource ty, Confirm the inference path doesn't invoke the LLM — the whole         point is t, Policy wildcards can't be safely auto-narrowed; we annotate with TODO. (+26 more)
...
Cohesion: 0.09
Nodes (34): Re-parse the patched output. Raise PatchValidationError if it fails., _validate_patch(), Any, parse_kubernetes_yaml(), Parse Kubernetes YAML (supports multi-document YAML)., _k8s_bundle(), Tests for Phase 3.4 Auto-Remediation.  Covers: - File discovery: finding -> file, Bug from samples/critical-security-failure.yaml: the LLM's     'Missing Terminat (+26 more)
...
Cohesion: 0.14
Nodes (28): CompanionResourceRequired, _looks_like_file_path(), NonPatchableFinding, PatchValidationError, True if the resource string looks like a Helm template path or a     file path r, Raised when remediation cannot proceed (no file match, no fixer, etc.)., The finding is advisory — it has no associated resource in any file     and is n, The finding requires creating a NEW Kubernetes resource alongside     the existi (+20 more)

### Community 10 - "Community 10"
Cohesion: 0.08
Nodes (14): is_non_patchable(), Return True for findings that don't map to any file-level edit.      These are t, _f(), End-to-end: LLM emits a patch with dash-line drift; the cleaned         diff has, The rule-based 'Security group open to 0.0.0.0/0' finding (which         is ingr, If the SG has no literal 0.0.0.0/0 (e.g. it uses var.allowed_cidrs),         the, Findings whose resource is N/A, empty, or a whole-infrastructure     sentinel ca, The bug from the screenshot: 'Lack of Commitment Discounts' with         resourc (+6 more)
...
Cohesion: 0.12
Nodes (11): Finding, extract_keywords(), is_duplicate(), Shared utility functions for agent deduplication., Insert spaces at camelCase and PascalCase boundaries.      'HorizontalPodAutosca, Extract significant keywords from text, with synonym expansion., Check if an LLM finding duplicates any rule finding using keyword overlap., _split_camelcase() (+3 more)

### Community 14 - "Community 14"
Cohesion: 0.12
Nodes (18): _build_control_assessability(), _classify_control(), _empty_mappings(), _entry_controls(), _entry_domain(), load_mappings(), Compliance framework mapping (Phase 3.3 — cloud-aware).  Tags every rule-based f, Map a control ID (e.g. "CIS-K8s-5.2.1") to its framework_id. (+10 more)

### Community 15 - "Community 15"
Cohesion: 0.15
Nodes (14): Synthesize all agent reports into final report., supervisor_node(), AnalysisReport, AgentReport, ArchitectureReview, ArchitectureReview, calculate_overall_score(), Calculate weighted overall score from agent reports + architecture review. (+6 more)
...
Cohesion: 0.14
Nodes (16): AgentReport, Plugin loader — runs discovered plugin agents and returns their reports.  Phase, Run a single plugin agent and return its report (or None if skipped)., Discover (or accept) plugins and run them sequentially.      Returns the list of, run_all_plugins(), run_plugin(), PluginAgent, A dynamically-registered analysis agent described by a skill file. (+8 more)

### Community 18 - "Community 18"
Cohesion: 0.14
Nodes (13): _detect_clouds(), _detect_clouds_from_resource(), Identify the cloud from a single Finding.resource string, or None.      K8s reso, Detect which clouds are present in the report.      Detection signals, in priori, _full_report(), Clean K8s upload (no findings) still detects kubernetes via extension., Phase 3.3 fix: a clean .tf file (no findings) must not falsely         imply AWS, Phase 3.3 regression: LLM-emitted findings sometimes use         ``resource="N/A (+5 more)

### Community 19 - "Community 19"
Cohesion: 0.13
Nodes (21): compute_agent_score(), Apply the standard deduction table to a list of Finding objects., Return a callable that reads a sample file from the repo's samples/ dir., sample_loader(), _parse_sample(), pytest_generate_tests(), Strict regression tests: run rule-based checks on each sample file and assert de, Parametrize the regression tests across every sample in the manifest. (+13 more)
...
Cohesion: 0.17
Nodes (17): AnalysisReport, Finding, AnalysisReport, ComplianceFrameworkScore, ComplianceScorecard, ComplianceScorecard, compute_compliance_scorecard(), _is_control_assessable() (+9 more)
...
Cohesion: 0.18
Nodes (10): Path, discover_plugins(), Scan the skills directory and return every eligible plugin agent.      Discovery, Path, No skills -> no reports (pass-through behavior)., The real skills/ dir must yield exactly the compliance plugin.          Core age, No discovered plugin maps to a hardcoded core agent., A skill lacking agent_type/weight is ignored by discovery. (+2 more)
...
Cohesion: 0.15
Nodes (6): Bug from samples/good-chart-1.1.0.tgz: the Reliability Agent's     rule-engine ', If the deployment already has a strategy block, the fixer         refuses to ove, Pods don't have an update strategy. If the LLM somehow emits         a strategy, The fixer adds default 25%/0 values — must surface a warning         so the user, End-to-end efficiency: strategy fixer must NEVER reach the         LLM. Set the, TestUpdateStrategyFixer

### Community 44 - "Community 44"
Cohesion: 0.38
Nodes (4): _dedup_cross_cutting_gaps(), Remove cross-cutting gaps that merely echo what individual agents already found., make_gap(), TestDedupCrossCuttingGaps
...
Cohesion: 0.27
Nodes (9): Any, Path, get_agent_prompt(), list_skills(), load_skill(), Skill file loader — reads agent behavior from markdown skill files., Load a skill file and return its metadata + prompt content.      Skill files use, Get the prompt for a specific agent and infrastructure type.      Args: (+1 more)
...
Cohesion: 0.32
Nodes (6): Any, _derive_agent_name(), _plugin_from_meta(), Plugin registry — discovers agent plugins from ``skills/*.md`` frontmatter.  Pha, Choose a display name for the agent.      Prefers an explicit ``agent_name`` in, Build a ``PluginAgent`` from one skill's frontmatter, or None if not eligible.

### Community 57 - "Community 57"
Cohesion: 0.25
Nodes (7): Architecture Decisions, Challenges Addressed, Components Delivered, Development Phases, Phase 3.5 — Plugin Harness (Dynamic Agent Registration) — SHIPPED 2026-07-15, Verification, What Was Built
...
## Knowledge Gaps
- **111 isolated node(s):** `Settings`, `Any`, `Path`, `Path`, `Any` (+106 more)
...
- **Why does `Severity` connect `Community 4` to `Community 0`, `Community 1`, `Community 5`, `Community 7`, `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 15`, `Community 16`, `Community 17`, `Community 19`, `Community 22`, `Community 24`, `Community 25`, `Community 26`, `Community 27`, `Community 29`, `Community 34`, `Community 35`, `Community 36`, `Community 39`, `Community 40`, `Community 42`, `Community 43`, `Community 44`, `Community 45`, `Community 46`, `Community 48`, `Community 49`, `Community 59`, `Community 60`, `Community 61`, `Community 62`?**
  _High betweenness centrality (0.160) - this node is a cross-community bridge._
- **Why does `Finding` connect `Community 4` to `Community 0`, `Community 1`, `Community 5`, `Community 7`, `Community 8`, `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 14`, `Community 15`, `Community 16`, `Community 17`, `Community 18`, `Community 22`, `Community 23`, `Community 24`, `Community 25`, `Community 26`, `Community 27`, `Community 29`, `Community 34`, `Community 36`, `Community 37`, `Community 40`, `Community 42`, `Community 43`, `Community 46`, `Community 48`, `Community 58`, `Community 59`, `Community 60`, `Community 61`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `sample_loader()` connect `Community 19` to `Community 28`, `Community 21`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Are the 84 inferred relationships involving `Severity` (e.g. with `CompanionResourceRequired` and `NonPatchableFinding`) actually correct?**
  _`Severity` has 84 INFERRED edges - model-reasoned connections that need verification._
- **Are the 76 inferred relationships involving `Finding` (e.g. with `CompanionResourceRequired` and `NonPatchableFinding`) actually correct?**
  _`Finding` has 76 INFERRED edges - model-reasoned connections that need verification._
